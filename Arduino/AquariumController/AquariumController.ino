...
void enterSleep() {
  if(AQUA_DEBUG_MODE_ON == 1) Serial.println("");Serial.println("Sleep...");
  ...
  if(AQUA_DEBUG_MODE_ON == 1) Serial.println("");Serial.println("Wakeup...");
}
  ...
  if(Event.reportObject.cmd == GENIE_REPORT_EVENT) {
    if(Event.reportObject.object == GENIE_OBJ_4DBUTTON) {
      ...
      if(Event.reportObject.index == GENIE_BTN_SLEEP) {
        objLCD.setActualForm(GENIE_FORM_SLEEP);
        genieWriteObject(GENIE_OBJ_FORM, GENIE_FORM_SLEEP, 0); //open screen saver form
        genieWriteContrast(0);
        wakeupLCD = 0;
      } else {
        if(Event.reportObject.index == GENIE_BTN_SET_TIME) {
          objLCD.setActualForm(GENIE_FORM_SET_TIME);
          keyboardStart = true;
          keyboardString = "0";
          settingsDatetime = objTIME.getDateTime();
          settingsTimeout = objLCD.getTimeout();
          genieWriteObject(GENIE_OBJ_FORM, GENIE_FORM_SET_TIME, 0);
          ...
          /*if((Event.reportObject.index == GENIE_BTN_CALIBRATION_TEMP && objTEMP.getTemp() > 0)
            ...
            keyboardStart = true;
            keyboardString = "0";
            calibration = Event.reportObject.index - GENIE_CALIBRATION_TEMP;
            genieWriteObject(GENIE_OBJ_FORM, GENIE_FORM_CALIBRATION, 0);
            genieWriteObject(GENIE_OBJ_STRINGS, GENIE_STR_INFO_CALIBRATION, 0);
            genieWriteObject(GENIE_OBJ_STRINGS, GENIE_STR_CAL_INFO, Event.reportObject.index - GENIE_BTN_CALIBRATION_TEMP);
            sprintf(row, "%5d\0", 0);
          ...
          objLCD.setActualForm(GENIE_FORM_SET_ALARM);
          keyboardStart = true;
          keyboardString = "0";
          settingsRelay = Event.reportObject.index - GENIE_BTN_SET_R1;
          settingsAlarm = objRELAY.readRelayAlarm(settingsRelay);
          genieWriteObject(GENIE_OBJ_FORM, GENIE_FORM_SET_ALARM, 0);
          ...
          objLCD.setActualForm(GENIE_FORM_SET_TIMER);
          keyboardStart = true;
          keyboardString = "0";
          settingsRelay = Event.reportObject.index - GENIE_BTN_SET_R1;
          genieWriteObject(GENIE_OBJ_FORM, GENIE_FORM_SET_TIMER, 0);
          ...
          objLCD.setActualForm(GENIE_FORM_MAIN);
          ...
          if(Event.reportObject.index == GENIE_BTN_CAL1_READ_ACT) {
            genieWriteStr(GENIE_STR_CAl1_ACT_NEW, row);
          } else if(Event.reportObject.index == GENIE_BTN_CAL2_READ_ACT) {
            genieWriteStr(GENIE_STR_CAl2_ACT_NEW, row);
          } else if(Event.reportObject.index == GENIE_BTN_CAL3_READ_ACT) {
            genieWriteStr(GENIE_STR_CAl3_ACT_NEW, row);
          }
      ...
      if(Event.reportObject.index == GENIE_BTN_WAKEUP) {
        objLCD.setActualForm(GENIE_FORM_MAIN);
        genieWriteObject(GENIE_OBJ_FORM, GENIE_FORM_MAIN, 0); //open main form
...
void setup() {
  ...
  if(AQUA_DEBUG_MODE_ON == 1) Serial.println("Temperature object initialization...");
  ...
  if(AQUA_DEBUG_MODE_ON == 1) Serial.println("Setup Watchdog Timer...");
  setupWDT();
  ...
  wakeup = 1;
  wakeupLCD = 1;
  if(AQUA_DEBUG_MODE_ON == 1) Serial.println("Ready for use!");
  delay(100);
}
...
void loop() {
  ...
  if(wakeupButton == 1) {
    ...
    wakeup = 1;
    inactivity = millis();
    if(AQUA_DEBUG_MODE_ON == 1) Serial.println("");Serial.println("pin2Interrupt...");
    if(wakeupLCD == 0) {
      objLCD.setActualForm(GENIE_FORM_MAIN);
      genieWriteObject(GENIE_OBJ_FORM, GENIE_FORM_MAIN, 0); //open main form
      genieWriteContrast(1);
